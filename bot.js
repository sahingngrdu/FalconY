...
if (config.TOKEN || process.env.TOKEN) {
  client.login(config.TOKEN || process.env.TOKEN).catch((e) => {
    console.log('Kronix | TOKEN ERROR❌❌');
  });
} else {
  setTimeout(() => {
    console.log('Kronix | TOKEN ERROR❌❌');
...
if(config.mongodbURL || process.env.MONGO){
  ...
  useUnifiedTopology: true,
    ...
    console.log('Kronix', `|    🍔 Connected MongoDB!`)
  }).catch((err) => {
    console.log('Kronix', `|    🍔 Failed to connect MongoDB!`)})
  } else {
  console.log('Kronix', `|    🍔 Error MongoDB!`)
  }
...
app.listen(port, () => {
  console.log(`Kronix | 🔗 Listening to Team Kronix`);
});
printWatermark();
